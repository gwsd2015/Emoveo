...
print "Doing initial serach for named entities...\n";
my $nerfile = "C:/Perl/namedentity.txt";
open(NER, "<", $nerfile) or die;
while(my $names = <NER>){
	chomp $names;
	my @work = split(/\n/, $names);
	push @namesearch, @work;
}

my @namesforconcordance;
my @comparearray = split(/\W/, $datatomod);

for(my $ix = 0; $ix < $#comparearray; $ix++){
	my $wordcomp = $comparearray[$ix];
	for(my $iy = 0; $iy < $#namesearch; $iy++){
		my $name_to_seek = $namesearch[$iy];
		if(length($name_to_seek) > 4){
			if(distance($name_to_seek, $wordcomp) < 4){
				push @namesforconcordance, $wordcomp;
			}
		}
		else{
			if($wordcomp eq $name_to_seek){
				push @namesforconcordance, $wordcomp;
			}
		}
	}
...
print "\nWould you like to remove names?\t";

my $nameyesno = <STDIN>;
chomp $nameyesno;
if($nameyesno =~ /yes/i){
	for(my $iz = 0; $iz < $#namesforconcordance; $iz++){
		my $nametoremove = $namesforconcordance[$iz];
		$datatomod =~ s/\b$nametoremove\b/||||||||||/ig;
	}
}

print "\nNames removed were...\n";
print "$_\n" for @namesforconcordance;
