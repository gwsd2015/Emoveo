...
print "Enter txt file locations (delimit files by semicolon):\t";
my $inputfiles = <STDIN>;
chomp $inputfiles;
...
for ( my $ff = 0 ; $ff < $#files ; $ff++ ) {

    my $inputfile = $files[$ff];
    ...
    my $num_paragraphs = readability($inputfile);
...
#need to use a transliterated list or Regexp::Common to identify quotes and parenthesis
    ...
    my $rtf = RTF::Writer->new_to_file("C:/Perl/".$inputfile."redacted.rtf");
...
"\n\nAre you finished with your redaction? If not, you can save a text file to continue redaction later.\t";
    ...
    print "\nScore difference report: \n";

print "Number of characters\t:\t".($num_chars2-$num_chars);
print "\nNumber of words\t:\t".($num_words2-$num_words);
print "\nPercent of complex words\t:\t".($percent_complex_words2-$percent_complex_words);
print "\nAverage of syllables per word\t:\t".($syllables_per_word2-$syllables_per_word);
print "\nNumber of sentences\t:\t".($num_sentences2-$num_sentences);
print "\nAverage words per sentence\t:\t".($words_per_sentence2-$words_per_sentence);
print "\nNumber of text lines\t:\t".($num_text_lines2-$num_text_lines);
print "\nNumber of blank lines\t:\t".($num_blank_lines2-$num_blank_lines);
print "\nNumber of paragraphs\t:\t".($num_paragraphs2-$num_paragraphs);

print "\n\nREADABILITY INDICIES\n";
print "Fog\t:\t".($fog2-$fog);
print "\nFlesch\t:\t".($flesch2-$flesch);
print "\nFlesch-Kincaid\t:\t".($kincaid2-$kincaid);
=cut
...
#http://stackoverflow.com/questions/18103501/prompting-multiple-questions-to-user-yes-no-file-name-input
